...
public class UserService {
     ...
     * @throws CustomException 사용자 ID가 이미 존재할 경우 (ErrorCode.BAD_REQUEST)
     */
    @Transactional
    public UserQueryResult registerUser(RegisterUserCommand command) { // 파라미터 이름을 'command'로 변경
        ...
        if (userRepository.existsById(userId)) {
            throw new CustomException(ErrorCode.BAD_REQUEST, "이미 존재하는 사용자 ID입니다: " + userId);
        }
...
//     * @param userId 조회할 사용자의 ID
     * @return 조회된 User 객체
     * @throws CustomException 사용자를 찾을 수 없을 때 (ErrorCode.USER_NOT_FOUND)
     */
    public UserQueryResult getUser(String userId) {
     ...
     * @throws CustomException 사용자를 찾을 수 없거나 금액이 유효하지 않을 때
     */
     ...
     * @param amount 사용할 금액
     * @return 업데이트된 User 객체
     * @throws CustomException 사용자를 찾을 수 없거나 잔액이 부족할 때
     */
    @Transactional(isolation = Isolation.READ_COMMITTED)
    public User useBalance(String userId, BigDecimal amount) {
        if (amount.compareTo(BigDecimal.ZERO) <= 0) {
            throw new CustomException(ErrorCode.INVALID_USE_AMOUNT);
        }
