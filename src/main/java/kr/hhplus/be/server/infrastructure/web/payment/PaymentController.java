...
import kr.hhplus.be.server.infrastructure.web.payment.dto.PaymentWebRequest;
import kr.hhplus.be.server.infrastructure.web.payment.dto.PaymentWebResponse;
...
public class PaymentController {
     ...
     */
    @PostMapping
    public ResponseEntity<PaymentWebResponse> processPayment(@Valid @RequestBody PaymentWebRequest request) {
        try {
            Payment payment = paymentService.processPayment(request.getUserId(), request.getReservationId(), request.getAmount());
            return new ResponseEntity<>(PaymentWebResponse.from(payment), HttpStatus.OK);
    ...
    public ResponseEntity<PaymentWebResponse> getPayment(@PathVariable String paymentId) {
...
//                .orElseGet(() -> new ResponseEntity<>(HttpStatus.NOT_FOUND));
        ...
        PaymentWebResponse response = PaymentWebResponse.from(payment);
    ...
    public ResponseEntity<List<PaymentWebResponse>> getPaymentsByUserId(@PathVariable String userId) {
        List<Payment> payments = paymentService.getPaymentsByUserId(userId);
        List<PaymentWebResponse> responses = payments.stream()
                .map(PaymentWebResponse::from)
